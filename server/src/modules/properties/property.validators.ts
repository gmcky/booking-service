...
export const createPropertySchema = z
  .object({
    ...
    apartment: z.string().trim().min(1).max(20).optional(),
...
export const updatePropertySchema = z
  .object({
    ...
    district: z.string().min(2).max(100).nullish(),
    street: z.string().trim().min(2).max(200).optional(),
    // Nullable so hosts can clear a house/apartment number they added by mistake.
    houseNumber: z.string().trim().min(1).max(20).nullish(),
    apartment: z.string().trim().min(1).max(20).nullish(),
    latitude: z.number().min(-90).max(90).optional(),
    longitude: z.number().min(-180).max(180).optional(),
