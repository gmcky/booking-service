...
describe("SearchPill guest stepper", () => {
  ...
  it("sums adults + children into the guest count sent, excludes infants, and clamps adults to a minimum of 1 once children/infants are added", async () => {
    renderPill();
    ...
    expect(stepperValue("Infants")).toBe("0");
    ...
    await userEvent.click(screen.getByRole("button", { name: "Increase children" }));
    await userEvent.click(screen.getByRole("button", { name: "Increase infants" }));
    expect(stepperValue("Children")).toBe("2");
    expect(stepperValue("Infants")).toBe("1");

    // Adults can't be decremented below 1 while children/infants are present.
    ...
    // Guest count sent = adults + children (2 + 2 = 4) — infants excluded.
    ...
    expect(pushedUrl).not.toMatch(/infants|children/);
  });
});
